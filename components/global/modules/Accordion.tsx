...
const AccordionComponent = ({ data }: { data: AccordionType }) => {
  return (
    <section
      className={clsx(
        'px-[24px] py-[66px]',
        'lg:grid lg:grid-cols-12 lg:gap-x-[24px] lg:px-[48px] lg:py-[89px]',
      )}
    >
          ...
          {data?.accordionItems?.map((item, index) => (
            ...
            >
              <Accordion.Header className={clsx('text-black')}>
                <Accordion.Trigger
                  className={clsx(
                    'flex justify-between gap-x-[16px] w-full py-[16px] border-t-[1px] border-t-lightGrey transition-colors duration-200',
                    'lg:py-[32px]',
